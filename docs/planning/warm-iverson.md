...
- **2026-04-18 — Session 1, ATM10-blueprint pivot + 4 verifications complete (R-ATM10):**
  ...
  - **STRIKE from ATM10** (~37–40 mods): all magic (~25, full Ars + Iron's Spells + Occultism + EvilCraft + Forbidden+Arcanus + Mahou Tsukai + Theurgy + Roots Classic + Nature's Aura + Relics + Reliquary Reincarnations + Apothic Enchanting + Gateways + Hostile Neural Networks + Blue Flame Burning + Potions Master), magic-adjacent endgame (Draconic Evolution + IceAndFire CE + L_Ender's Cataclysm), off-theme dimensions (The Aether + Twilight Forest + Eternal Starlight + Deeper and Darker + Bumblezone + Undergarden), redundant (Security Craft), Mineflayer-caveat (Not Enough Animations + playerAnimator + Cosmetic Armor Reworked — pending verify).
  - **Final pack-size estimate: ~470 mods** (~500 ATM10 − ~37–40 strikes + ~8 monopoly-layer adds).
  - **VS2 caveat:** R-ATM10 claimed VS2 is hard dep of Create: Aeronautics; this **appears incorrect** (Aeronautics is Create-native physics; VS2 was Create: Interactive's dep, and Interactive is dropped). Action: confirm Aeronautics 1.0.2 dep tree before locking VS2 in or out.
