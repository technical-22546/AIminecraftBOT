...
- **2026-04-18 — Session 1, ATM10-blueprint pivot + 4 verifications complete (R-ATM10):**
  - **Strategic pivot LOCKED:** use **ATM10 (~450 mods) as the shortlist blueprint** rather than building bottom-up. Start from ATM10's NF 1.21.1 list, strike magic + Mineflayer-breaking + theme-incompatible, overlay monopoly-specific custom layer. Cuts authoring load massively; inherits tested mod combinations.
  - **All 4 additions verified SHIPPING:** Modern Industrialization v2.4.2 (native NeoForge), Integrated Dynamics suite all 5 (ID 1.26.1 + Tunnels 1.8.28 + Terminals 1.6.15 + Crafting + Scripting; needs Common Capabilities + Cyclops Core; **AE2 Spatial IO incompat — disable spatial transport of ID networks**), Extreme Reactors 2.4.28 (drop-in Bigger Reactors swap), Easy Villagers 1.21.1-1.1.39 (Minecolonies QoL).
  - **MUST ADD on top of ATM10** (monopoly layer): Create: Aeronautics (core identity), Create Railways Navigator, Minecolonies: War 'N Taxes, SDMShop, Tectonic, Terralith datapack (if not via Lithostitched), Guard Villagers, Joe AI external stack.
  - **STRIKE from ATM10** (~30–32 mods after user follow-up): all spell/altar/familiar magic (~20: full Ars + Iron's Spells + Iron's Gems 'n Jewelry + Occultism + EvilCraft + Forbidden+Arcanus + Mahou Tsukai + Theurgy + Roots Classic + Nature's Aura + Relics + Reliquary Reincarnations + Blue Flame Burning + Potions Master + StarbuncleMania + Not Enough Glyphs + All The Arcanist Gear + All the Wizard Gear), magic-adjacent (IceAndFire CE), off-theme dimensions (The Aether + Twilight Forest + Eternal Starlight + Deeper and Darker + Bumblezone + Undergarden), redundant (Security Craft), Mineflayer-caveat (Not Enough Animations + playerAnimator + Cosmetic Armor Reworked — pending verify).
  - **KEPT per user follow-up 2026-04-18** (initially struck, then restored): **Hostile Neural Networks**, **Gateways to Eternity**, **Draconic Evolution**, **L_Ender's Cataclysm**, **Apothic Enchanting**. User distinguishes RPG/loot/affix/enchant mechanics (in) from spell-casting magic (out).
  - **Final pack-size estimate: ~475 mods** (~500 ATM10 − ~30–32 strikes + ~8 monopoly-layer adds).
